...
bool test_digit(char value) { return std::isdigit(value); }

struct NumberClass {
  bool is_number(char x) { return test_digit(x); }
};

METHODE_TEST_CASE(TEST_CLASS_METHOD, NumberClass, {}, is_number, (false, 'a'),
                  (true, '1'), (false, 'b'));
FUNCTION_TEST_CASE(IS_NUMBER, test_digit, (false, 'a'), (true, '1'),
                   (false, 'b'));

SIMPLE_TEST_CASE(LOG_BOOK_TEST) {
  using purge::to_string;
  REQUIRE(to_string(purge::TEST_STATE::SEGMENTATION_FAULT) ==
...
/*FUZZER_CONTAINER_CASE(IS_SUM_GT_FUZZ, std::vector<int>,
                      purge::item_rule<int>([](int &&v) -> bool {
                        return v > 0;
                      }),
                      FZ) {
...
/*FUZZER_CONTAINER_CASE(FUZZ_HAS_NES_LINE, std::string,
                      purge::item_rule<char>([](char &&v) -> bool {
                        return true;
                      }),
                      FV) {
  hasNewLine(FV);
